...
const FindPage = {
  async render() {
    ...
    for (const cat of cats) {
      for (const s of cat.services) {
        if (s.name.toLowerCase().includes(ql) || cat.name.toLowerCase().includes(ql)) {
          selectedService = s.id; this._svcName = s.name; break;
        }
      }
    ...
    const locOptions = localities.map(l => `<option value="${esc(l)}">${esc(l)}</option>`).join("");

    el.innerHTML = `
      <form id="filter-form" class="card mb-2">
        <div class="field-row">
          <div class="field span-2"><label for="f-svc">Service</label>
            <select id="f-svc" class="input">${serviceOptions}</select></div>
    ...
    const applySearch = (svcId) => { if (svcId) serviceSel.value = svcId; runResults(); runShortlist(); };
    document.getElementById("filter-form").addEventListener("submit", e => { e.preventDefault(); runResults(); runShortlist(); });

    async function runResults() {
      const svc = serviceSel.value || null;
      const loc = document.getElementById("f-loc").value || null;
      const budget = parseFloat(document.getElementById("f-budget").value) || null;
      let arr = providers.filter(p => !loc || (p.locality || "").toLowerCase() === loc.toLowerCase());
      if (svc) arr = arr.filter(p => p.services.some(s => s.service_id == svc));
      if (budget != null) arr = arr.filter(p => p.services.some(s => s.hourly_rate != null && s.hourly_rate <= budget));
    ...
    async function runShortlist() {
      ...
      if (budget != null) bp.push("max_budget=" + budget);
    ...
    function renderResults(arr) {
      ...
      if (!arr.length) { box.innerHTML = `<div class="state-box"><div class="big">🔍</div>No professionals match your filters. Try widening locality or budget.</div>`; return; }
      box.innerHTML = arr.map(p => `
        <div class="list-item">
          ...
          <div style="text-align:right"><div class="price">${money(p.services[0] && p.services[0].hourly_rate)}<small>/hr</small></div>
