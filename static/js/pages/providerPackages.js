// CrewNest Atlas offers — service package catalog.
  ...
  async render() {
    ...
    el.innerHTML = `
      <header class="atlas-pagehead">
        <div><div class="atlas-kicker">Offer shelf</div><h1>Offers</h1><p>Build clear service bundles customers can understand and book quickly.</p></div>
        <div class="atlas-actions"><span class="atlas-tag blue">${active.length} published</span><span class="atlas-tag">${teams.length} team</span><span class="atlas-tag">${multi.length} multi</span><button class="atlas-btn primary" id="create-pkg">${icon("plus")} New offer</button></div>
      </header>

      <section class="atlas-package-grid">${pkgs.length ? pkgs.map(p=>this.card(p)).join("") : `<div class="atlas-card atlas-empty" style="grid-column:1/-1"><div><strong>No offers yet</strong><span>Create your first multitasking or team package.</span></div></div>`}</section>
    `;
    ...
    el.querySelector("#create-pkg").addEventListener("click",()=>this._modal(svcs,null));
  },

  card(p){
    const members = p.member_count ? `${p.member_count} member crew` : "Solo delivery";
    return `<article class="atlas-card atlas-offer ${p.package_type === "team" ? "team" : ""}"><div class="between"><span class="atlas-offer-type">${p.package_type === "team" ? "TEAM OFFER" : "MULTITASKING"}</span><span class="atlas-tag ${p.status === "published" ? "mint" : ""}">${esc(p.status)}</span></div><h3>${esc(p.name)}</h3><p>${esc(p.locality)} · ${p.service_count} services · ${members}</p><div class="atlas-tags">${p.services.slice(0,4).map(s=>`<span class="atlas-tag">${esc(s)}</span>`).join("")}</div><div class="atlas-offer-rate">${money(p.hourly_rate)}<small>/hr</small></div>${p.status === "archived" ? `<p>Archived — existing bookings stay unchanged.</p>` : `<div class="atlas-offer-actions"><button class="atlas-btn light" data-edit="${p.id}">Edit offer</button><button class="atlas-btn danger" data-archive="${p.id}">Archive</button></div>`}</article>`;
  },

  _modal(svcs,pkg){
    const me=Auth.get();
    const wrap=document.createElement("div");
    wrap.className="dialog-backdrop open atlas-provider-dialog";
    wrap.setAttribute("role","dialog"); wrap.setAttribute("aria-modal","true");
    const initialType=pkg ? pkg.package_type : "multitasking";
    wrap.innerHTML=`<div class="dialog"><div class="atlas-kicker">${pkg ? "Edit offer" : "New offer"}</div><h2>${pkg ? "Refine your offer" : "Build a bookable offer"}</h2><p class="small muted">Define the outcome, location, services, team and whole-package hourly rate.</p><form id="pkg-form" novalidate>
      <div class="field"><label>Package type</label><div class="chips-row" id="type-tabs"><button type="button" class="pill-btn ${initialType === "multitasking" ? "active" : ""}" data-type="multitasking">Multitasking</button><button type="button" class="pill-btn ${initialType === "team" ? "active" : ""}" data-type="team">Team</button></div></div>
      ...
      <div class="dialog-actions"><button class="atlas-btn light" id="pm-close" type="button">Cancel</button><button class="atlas-btn primary" id="pm-submit" type="submit">${pkg ? "Save changes" : "Publish offer"}</button></div>
    </form></div>`;
    ...
    wrap.querySelector("#pkg-form").addEventListener("submit",async e=>{
      ...
      try{if(pkg)await API.put("/api/providers/me/packages/"+pkg.id,payload);else await API.post("/api/providers/me/packages",payload);Toast.success(pkg?"Offer updated":"Offer published");close();this.render();}catch(err){Toast.error(err.message);btn.disabled=false;}
    });
  },

  async _archive(id,btn){if(!confirm("Archive this package? New bookings will be blocked; existing bookings stay unchanged."))return;btn.disabled=true;try{await API.post("/api/providers/me/packages/"+id+"/archive",{});Toast.success("Offer archived");this.render();}catch(e){Toast.error(e.message);btn.disabled=false;}}
};
